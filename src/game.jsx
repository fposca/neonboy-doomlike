...
export default function Game() {
  ...
  useEffect(() => {
    ...
    function makeMap(w,h){
      ...
      const blocks = Math.floor(w*h*0.06);
    ...
    const CONTACT_DPS = 25; // (sin uso directo ahora)
    const BACKOFF_DIST = 0.4, BACKOFF_TIME = 0.5, TOUCH_COOLDOWN = 0.6;

    function makeEnemy(x, y, type = (Math.random()<0.7 ? "demon" : "orc")) {
      const baseHp = (type === "orc") ? 80 : 40; // orco más duro
      return {
        x, y, hp: baseHp, alive: true, radius: 0.3,
    ...
    let hitFlash = 0;
    const HIT_FLASH_TIME = 0.25; // segundos
    ...
    Promise.all([
      ...
      pngAmmo, pngHeart, pngGreen, pngShells, brickRed, brickPink,
      ...
      TEX[1]=brickRed||null; TEX[2]=brickPink||null;
    ...
    const endGame = (v)=>{ gameOver=true; win=v; gameState = v ? "win" : "gameover"; };
    ...
    function damageEnemy(e,dmg){
      ...
      if(e.hp<=0){
        ...
        if (e.type === "orc") killsOrcs++; else killsDemons++;
        SFX.enemyDie();
    ...
    function tick(now){
      ...
      if(!gameOver){
        ...
        if (spawnTimer >= SPAWN_EVERY && enemies.length < MAX_ENEMIES) {
          ...
          if (p) {
            const wasEmpty=enemies.length===0;
            enemies.push(makeEnemy(p.x,p.y)); // 70% demon / 30% orc
            SFX.enemySpawn();
            if (wasEmpty) startBreathLoop(performance.now());
          }
          spawnTimer = 0;
        ...
        for(const e of enemies){
          ...
          const dx = Math.cos(ang) * ENEMY_SPEED * dt;
          const dy = Math.sin(ang) * ENEMY_SPEED * dt;
          slideMoveEntity(e, dx, dy);

          // --- Daño por toque: 10 por golpe con cooldown ---
          const d = dist(player.x,player.y,e.x,e.y);
          if (d < 0.6 && e.touchCd <= 0) {
            player.hp = Math.max(0, player.hp - 10);
            setFaceState("hurt", face.HURT_TIME);
            hitFlash = HIT_FLASH_TIME; // activar flash rojo
            SFX.hurt();                // <<< sonido de daño
    ...
    function drawHUD(){
      ...
      const have1 = true;
      const have2 = player.ammo   > 0;
      const have3 = player.shells > 0;
    ...
    function drawFrame(hShift){
      ...
      bctx.fillStyle="#1a1a1a"; bctx.fillRect(0,0,W, clampf(horizon,0,H));
      const grad=bctx.createLinearGradient(0,horizon,0,H); grad.addColorStop(0,"#151515"); grad.addColorStop(1,"#050505");
      bctx.fillStyle=grad; bctx.fillRect(0,horizon,W,H-horizon);
      ...
      if (gameOver){
        ...
        bctx.fillStyle=win?"#6f6":"#f66"; bctx.font="bold 16px 'Roboto', monospace";
        const txt = win ? "¡VICTORIA!" : "GAME OVER";
